...
if [[ "$IS_C_PROGRAM" == "yes" ]] ; then
   ...
   >&2 echo "  1 - Assembling test case"
   ./test/tools/bin/assembler < ./test/assembly/${TESTCASE}.asm > ./test/binary/${TESTCASE}.hexle
   sed 's/\(..\)\(..\)\(..\)\(..\)/\4\3\2\1/g' ./test/binary/${TESTCASE}.hexle > ./test/binary/${TESTCASE}.hex
else
...
if [[ "$IS_C_PROGRAM" == "yes" ]] ; then
   ...
   ./test/tools/bin/asm_reference_simulator < ./test/binary/${TESTCASE}.hexle > ./test/reference/${TESTCASE}.out-v0
fi
RESULT=$?
set -e
